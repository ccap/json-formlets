...
object Formlet extends FormletSyntax {
  ...
  implicit class FormletValidated[M[_], I, V, E, A](val self: Formlet[M, Validated, I, V, E, A])
    extends AnyVal {
    ...
    def mapValidation[B](
      ...
      self.mapResult((a, v) => ((a.toEither.flatMap(f(_).toEither)).toValidated, v))
    ...
    def flatMap[B](
      ...
      Formlet(i =>
        M.flatMap(self.run(i)) {
          case (Left(e), v1) => M.point((Left(e), v1))
          case (Right(a), v1) =>
            M.map(f(a).run(i)) {
              case (r2, v2) =>
                (r2, V.combine(v1, v2))
  ...
  implicit def formletApplicative[
    ...
    new Applicative[Formlet[M, N, I, V, E, ?]] {
      ...
      override def pure[A](a: A): Formlet[M, N, I, V, E, A] =
        Formlet[M, N, I, V, E, A](_ => Applicative[M].point((NN.point(a), Monoid[V].empty)))
  ...
  implicit def formletDisjunctionMonad[
    ...
    new Monad[Formlet[M, Either, I, V, E, ?]] {
      ...
      override def pure[A](a: A): Formlet[M, Either, I, V, E, A] =
        Formlet[M, Either, I, V, E, A](_ => Monad[M].point((a.asRight, Monoid[V].empty)))
      ...
      ): Formlet[M, Either, I, V, E, B] = scala.Predef.???
    }

  def ask[M[_]: Applicative, N[_, _], I, V: Monoid, E](
    implicit NN: Applicative[N[E, ?]],
  ): Formlet[M, N, I, V, E, I] =
    Formlet(i => Applicative[M].point((NN.point(i), Monoid[V].empty)))
  ...
  def point[M[_]: Applicative, N[_, _], I, V: Monoid, E, A](
    ...
    Formlet(_ => Applicative[M].point((NN.point(a), Monoid[V].empty)))
