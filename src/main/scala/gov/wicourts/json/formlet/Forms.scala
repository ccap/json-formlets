...
object Forms {
  ...
  private def primitive[M[_]: Applicative, A](
    ...
    Formlet { c =>
      val result = OptionT(
        c.flatMap(_.downField(name))
          ...
          j.asRight[NonEmptyList[String]]
            .ensure(
              NonEmptyList.of(s"Field $name must be a(n) $descr"),
            )(
              matches,
            )
            .flatMap(fromJson),
      ...
      Applicative[M].point((result, view))
    }
  ...
  private def traverseBreak[X](
    ...
    x => {
      ...
      def spin(z: X, d: Cursor): X = {
        val (q, k) = r.run(d).runF.value(z).value
        k match {
          case None => q
  ...
  def listM[M[_], A](
    ...
    Formlet { c =>
      ...
      l.zipWithIndex
        ...
        .sequence
        .map(_.sequence.map(_.sequence).swap)
    }
  ...
  def label[M[_]: Functor, A](
    ...
    field.mapView(_.copy(label = label.some))

  def errorName[M[_]: Functor, A](
    field: FieldFormlet[M, A],
    errorName: String,
  ): FieldFormlet[M, A] =
    field.mapView(_.copy(errorName = errorName))

  def required[M[_]: Functor, A](
    field: FieldFormlet[M, Option[A]],
  ): FieldFormlet[M, A] =
    field.mapValidation((_: Option[A]).toValid(NonEmptyList.of("This field is required")))
  ...
  def requiredObj[M[_]: Functor, A](
    ...
    obj.mapValidation(
      (_: Option[A]).toValid(ValidationErrors.string(name, "This field is required")),
    )
