...
case class FieldErrors private[formlet] (errors: NonEmptyList[String]) extends ValidationErrors {
  def toJson: Json = Json.array(errors.map(jString(_)).toList: _*)
}

case class ObjectErrors private[formlet] (errors: List[(String, ValidationErrors)])
  extends ValidationErrors {
  def toJson: Json = Json.obj(errors.map { case (n, e) => (n, e.toJson) }: _*)
}

case class ArrayErrors private[formlet] (errors: List[(Int, ValidationErrors)])
  extends ValidationErrors {
...
object ValidationErrors {
  ...
  implicit val validationErrorsEqual: Eq[ValidationErrors] = Eq.instance((a1, a2) => {
    (a1, a2) match {
  ...
  implicit val validationErrorsMonoid: Monoid[ValidationErrors] = new Monoid[ValidationErrors] {
    ...
    private def merge[A: Eq](
      ...
      val (merged, remaining) =
        e1.foldLeft((List[(A, ValidationErrors)](), e2)) {
          case ((merged, remaining), (a, errors)) =>
            val (ours, others) = remaining.partition { case (a1, _) => a1 === a }
            ((a, NonEmptyList.of(errors, ours.foldMap(_._2)).fold) :: merged, others)
        }
      merged.reverse ++ remaining
    }
